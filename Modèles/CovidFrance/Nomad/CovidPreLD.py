# -*- coding: utf-8 -*-
"""
Created on Tue Oct 19 14:58:30 2021
...
def pre(region,seed,n_runs,x0):
    """
    beta,lag,delay,scale1=[0.085,26,8,1.4]
    """
    filename = x0
    #filename = sys.argv[1]
    ...
    beta=float(x[0]) #float
    lag=int(x[1]) #int   #number of days before March 1
    delay=int(x[2]) #int
    scale1=float(x[3]) #float
    
    #n_runs=500 #int
    #region='PDL'
    rng=np.random.default_rng(seed)
    
    list_regions = {'IDF': 11,'CVL':24, 'BFC':27, 'NOR': 28, 'HDF':32, 'GRE':44, 'PDL':52, 'BRE':53,'NAQ':75, 'OCC':76, 'ARA':84, 'PACA':93}
        # select region
    ...
    output = run_simulation(intervention=True,LD=[[start_ld+delay,m_11]],
                     scale1=scale1,scale2=[0 for i in range(len(weeks))],
                     x_test_a=[0 for i in range(len(weeks))],  
                     x_test_s=[0 for i in range(len(weeks))])  
    median_adm_H = add_median_CI(output['adm_H'])['median']
    p=0
    for t in range(71): # consider hospitalization data from March 1 to May 11
        #compute log likelihood
        p += poisson.logpmf(H_adm['obs'].iloc[t], mu=median_adm_H.iloc[t+lag])
    #print('Loglikelihood with the current set of 4 parameters: ',p)
    f=-p #On cherche à maximiser p donc à minimiser f=-p
    print(f)
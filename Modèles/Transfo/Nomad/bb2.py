...
def bb(): #x[0]=a x[1]=b x[2]=c x[3]=d x[4]=I2 x[5]=n1 x[6]=s1 x[7]=s2
    ...
    #on crée les symboles de variables non causales
    ...
    #on résout le système pour obtenir les valeurs des variables non causales
    ...
    Eqn2 = n2 - ((x[5]*(V2+DV2)/V1))
    sols = list(sympy.nonlinsolve([EqX2,EqQ1,EqDV2,Eqn2,Eqr1,Eqr2,EqTc,EqTi,EqPj,EqR2], [X2, Q1, DV2, n2, r1, r2, Tc, Ti, Pj, R2]))
    
    #On récupère les sorties du système s'il y en a, sinon on donne des valeurs barrière
    if (len(sols)==0):
        X2=1e20
        Q1=1e20
        DV2=1e20
    ...
    #On crée le vecteur sortie
    out=[f,c1,c2,c3,c4,c5,c6,c7,c8,c9]

    #on affiche le vecteur sortie pour l'execution
    print(out)
bb()

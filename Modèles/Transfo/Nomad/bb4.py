...
def bb(): #x[0]=a x[1]=b x[2]=c x[3]=d x[4]=I2 x[5]=n1 x[6]=s1 x[7]=s2
    ...
    t1 = (mu0*pow(n2,2)*c*(4*a+2*d+math.pi*c)*2*math.pi*freq/(3*b))
    t2 = (pow(V1,2)/(Lmu*2*math.pi*freq)+X2*pow(I2,2)+V2*I2*phi)
    t3 = (I2*(R2*fp+X2*phi))
    t4 = (rho*(1+alpha*Tc)*n1*l1/(s1))
    t5 = (rho*(1+alpha*Tc)*n2*l2/(s2))
    t6 = (Text + Rca*(Rcond*Pj+Ria*(Pj+Pi))/(Rca+Ria+Rcond))
    t7 = (Text + Ria*(Rca*Pj+(Rca+Rcond)*Pi)/(Rca+Ria+Rcond))
    t8 = (R2*pow(I2,2))
    t9 = (r2 + r1*pow(n2/n1,2))
    t10 = ((n1*(V2+DV2)/V1))

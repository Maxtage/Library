...
def bb(): #x[0]=a x[1]=b x[2]=c x[3]=d x[4]=I2 x[5]=n1 x[6]=s1 x[7]=s2
    ...
    #X2 = (mu0*pow((n1*((V2+1)/V1)),2)*c*(4*a+2*d+math.pi*c)*2*math.pi*freq/(3*b))
    #R2 = (rho*(1+alpha*100)*n1*l1/(s1))+pow((V2+1)/V1,2)*(rho*(1+alpha*100)*n1*((V2+1)/V1)*l2/(s2))
    ...
    X2 = (mu0*pow((n1*(V2+(I2*(R2prim*fp+X2prim*phi)))/V1),2)*c*(4*a+2*d+math.pi*c)*2*math.pi*freq/(3*b))
    R2 = ((rho*(1+alpha*(Text + Rca*(Rcond*R2prim*pow(I2,2)+Ria*(R2prim*pow(I2,2)+Pi))/(Rca+Ria+Rcond)))*(n1*(V2+(I2*(R2prim*fp+X2prim*phi)))/V1)*l2/(s2)) + (rho*(1+alpha*(Text + Rca*(Rcond*R2prim*pow(I2,2)+Ria*(R2prim*pow(I2,2)+Pi))/(Rca+Ria+Rcond)))*n1*l1/(s1))*pow(((V2+(I2*(R2prim*fp+X2prim*phi)))/V1),2))
        
    Eps = 1e-5
    compteur = 0
    
    while ((abs(X2prim-X2)/max(X2,X2prim)>=Eps and abs(R2prim-R2)/max(R2,R2prim)>=Eps) and compteur<100) :
